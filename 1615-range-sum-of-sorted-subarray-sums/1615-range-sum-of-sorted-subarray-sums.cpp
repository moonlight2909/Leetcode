...
private:
    ...
    countAndSumSubarrays(const std::vector<int>& array, long long threshold) {
        int count = 0;
        ...
        for (int start = 0, end = 0; end < size; ++end) {
            runningSum +=
                static_cast<long long>(array[end]) * (end - start + 1);
    ...
    long long calculateSumOfFirstKSubarrays(const std::vector<int>& array,
                                            int k) {
...
public:
    int rangeSum(std::vector<int>& nums, int n, int left, int right) {
        ...
        long long result =
            (calculateSumOfFirstKSubarrays(nums, right) % MODULUS -
