...
public:
    ...
    int help(int idx, int m, bool aliceTurn, vector<long long>& pref,
        ...
        if (aliceTurn) {
            ...
            for (int x = 1; x <= 2 * m; x++)
                if (idx + x - 1 < piles.size())
                    ans = max(ans, get(idx, idx + x - 1, pref) +
                                       help(idx + x, max(m, x), false, pref,
                                            piles, dp));
    ...
    int stoneGameII(vector<int>& piles) {
        ...
        vector<vector<vector<int>>> dp(
            n + 1, vector<vector<int>>(2 * n + 2, vector<int>(2, -1)));
